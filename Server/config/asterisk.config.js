...
// const {
//   createCall,
//   updateCallToAnsweredANdTimeById,
...
ari
  ...
  .then(function (client) {
    ...
    async function originate(channel, holdingBridge) {
      ...
      basicCall(channel, holdingBridge);
    }

    function basicCall(channel, holdingBridge) {
      console.log("bassssssssic");
      // Create a channel for the worker
      var dialed = client.Channel();
      TheOnetoCAll = "PJSIP/2000";
      channel.on("StasisEnd", function (event) {
        //close the worker phone
        safeHangup(dialed);
      });
      // initiate the worker phone to connect it to the customer phone
      dialedAttribute(client, TheOnetoCAll, channel, dialed, holdingBridge);
    }
    async function dialedAttribute(
      client,
      TheOnetoCAll,
      channel,
      dialed,
...
//         thischannel.on(
//           "StasisEnd",
//           function (event, thischannel, TheOnetoCAll) {
...
//     }
//     myArray = [customerId, thischannel.caller.number, TheOnetoCAll._id];
